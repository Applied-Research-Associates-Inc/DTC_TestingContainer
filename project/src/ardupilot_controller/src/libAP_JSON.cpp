...
bool libAP_JSON::InitSockets( const char* fdm_address, const uint16_t fdm_port_in )
{
    ...
    {
        std::cout << "[libAP_JSON] "
                  << "failed to bind with " << fdm_address << ":" << fdm_port_in << std::endl;
        return false;
    }
    std::cout << "[libAP_JSON] "
              << "flight dynamics model at " << fdm_address << ":" << fdm_port_in << std::endl;
    return true;
}
...
bool libAP_JSON::ReceiveServoPacket( uint16_t servo_out[] )
{
    ...
    else if ( pkt.frame_count != fcu_frame_count + 1 && ap_online )
    {
        ...
        // std::cout << "[libAP_JSON] Missed " << fcu_frame_count - pkt.frame_count << " input frames. Expected: " << fcu_frame_count + 1 << " got: " << pkt.frame_count << "\n";
    }
    ...
    if ( !ap_online )
    {
        ...
        std::cout << "[libAP_JSON] "
                  << "Connected to ArduPilot controller @ " << fcu_address << ":" << fcu_port_out << "\n";
    }
...
void libAP_JSON::SendState( double timestamp, // seconds
                            double gyro_x,
                            double gyro_y,
                            double gyro_z, // rad/sec
                            double accel_x,
                            double accel_y,
                            double accel_z, // m/s^2
                            double pos_x,
                            double pos_y,
                            double pos_z, // m in inertial frame
                            double phi,
                            double theta,
                            double psi, // attitude radians
                            double V_x,
                            double V_y,
                            double V_z ) // m/s (standard fixed wing frame is negative)
{
    ...
    std::string s = "\n{\"timestamp\":" + std::to_string( timestamp ) + ",\"imu\":{" + "\"gyro\":[" +
                    std::to_string( gyro_x ) + "," + std::to_string( gyro_y ) + "," +
                    std::to_string( gyro_z ) + "]" + ",\"accel_body\":[" + std::to_string( accel_x ) + "," +
                    std::to_string( accel_y ) + "," + std::to_string( accel_z ) + "]" + "}" +
                    ",\"position\":[" + std::to_string( pos_x ) + "," + std::to_string( pos_y ) + "," +
                    std::to_string( pos_z ) + "]" + ",\"attitude\":[" + std::to_string( phi ) + "," +
                    std::to_string( theta ) + "," + std::to_string( psi ) + "]" + ",\"velocity\":[" +
                    std::to_string( V_x ) + "," + std::to_string( V_y ) + "," + std::to_string( V_z ) + "]";
    ...
    {
        s = s + ",\"windvane\":{\"direction\":" + std::to_string( windvane_direction ) +
            ",\"speed\":" + std::to_string( windvane_speed ) + "}";
    }
    ...
    sock.sendto( s.c_str(), s.size(), fcu_address, fcu_port_out );
...
void libAP_JSON::setRangefinder( const double* rangefinder_in, uint8_t n )
{
    if ( n <= 6 )
    {
        rangefinder_count = n;
